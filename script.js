...
// just interested in knowing whether a dog is an adult or a puppy. A dog is an adult if it
...
// cats, not dogs! So create a shallow copy of Julia's array, and remove the cat ages from 
...
// const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // dogs.forEach(function (dog, i) {
    // if (dog >= 3) {
      // console.log(`Dog number ${i + 1} is an adlut, and is ${dog} years old`);
    // } else {
...
// 
// };
//checkDogs([3, 5, 2, 12, 7], [4, 1, 15, 8, 7]);
...
//const movements = [200, 450, -400, 3000, -650, -130, 70, 1300];
//const eurtoUsd = 1.1;
//movementsovements.map(function)
// 
// The new at Method
const arr = [23, 11, 64];
console.log(arr[0]);
...
years old, humanAge = 2 * dogAge. If the dog is > 2 years old, humanAge = 16 + dogAge * 
4.
...
const calcAverageHumanAge = function (ages) {
  ...
  const adults = humanAges.filter(age => age >= 18);// it will check age greater than 18 and filter 
...
console.log(movements);
// Pipelines
const totalDepositesUSD = movements.filter(mov => mov > 0)
  .map((mov, i, arr) => {
  //console.log(arr);
  return mov * eurToUsd;
})
...
const calcAverageHumanAge2 = function (ages) {
  const humanAges = ages.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
  const adults = humanAges.filter(age => age >= 18);// it will check age greater t
